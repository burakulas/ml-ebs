...
# =============================================================================
WORK_DIR = "."
CUSTOM_LC_DIR = os.path.join(WORK_DIR, "custom_data")
...
# =============================================================================
# 2. ROBUST LOADER (CSV with phase/flux headers)
# =============================================================================
def load_and_bin_lc_robust(filepath, n_bins=1000):
    """
    Load a user-provided light curve CSV with 'phase' and 'flux' columns.
    Phase is folded to [0, 1), then binned, smoothed, and shifted to [0.25, 1.25].
    """
    try:
        ...
        df = df[['phase', 'flux']].apply(pd.to_numeric, errors='coerce').dropna()
        if len(df) < 10:
            return None

        raw_phase = df['phase'].values % 1.0
...
# =============================================================================
# 3. FEATURE EXTRACTION (51 features — identical to 2_extract_training_features.py)
# =============================================================================
...
def main():
    ...
    all_files = [f for f in os.listdir(CUSTOM_LC_DIR) if f.lower().endswith(('.csv', '.dat'))]
    ...
    for i in tqdm(range(0, len(all_files), BATCH_SIZE), desc="Batches"):
        ...
        for f_name in batch_files:
            flux = load_and_bin_lc_robust(os.path.join(CUSTOM_LC_DIR, f_name))
            if flux is not None:
